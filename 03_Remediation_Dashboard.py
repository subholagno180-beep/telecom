...
with tabs[0]:
    if st.button('Remediation Agent'):
        st.write('Running Remediation Agent')
        st.write('Solution of the issue with reference')
        st.write('Major or complex solution or minor')
        st.write('if major inserted in defect logs')
        st.write('minor start human approval section')
with tabs[1]:
    st.pdf(SOLM_PATH)
